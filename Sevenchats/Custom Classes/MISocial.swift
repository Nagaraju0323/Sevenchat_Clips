...
extension MISocial
{
    func twitterLogin(fromVC viewController : UIViewController?, completion userInfoBlock:TwiterUserInfoBlock?) {
        twiterUserInfoBlock = userInfoBlock
        ...
        TWTRTwitter.sharedInstance().logIn(with: viewController) { (session, error) in
            guard self.twiterUserInfoBlock  != nil else {
                return
            }
            if self.twiterUserInfoBlock != nil{
                self.twiterUserInfoBlock!(session, error)
                self.twiterUserInfoBlock = nil
            }
        }
    }
    
    func twitterEmailRequest(userId : String?, completion emailInfoBlock:TwiterEmailInfoBlock?)
    {
        twiterEmailInfoBlock = emailInfoBlock
...
//        let client = TWTRAPIClient.withCurrentUser()
        client.requestEmail { email, error in
            if (email != nil) {
                if self.twiterEmailInfoBlock != nil{
                    self.twiterEmailInfoBlock!(email, error)
                    self.twiterEmailInfoBlock = nil
                }
            } else {
                if self.twiterEmailInfoBlock != nil{
                    self.twiterEmailInfoBlock!(nil, error)
                    self.twiterEmailInfoBlock = nil
                }
            }
        }
    ...
    func openTwitterComposer(text : String?, image : UIImage?, fromVC viewController : UIViewController?){
        // Swift
    ...
    func twitterFriendList(cursor : String?, fromVC viewController : UIViewController?,friendListBlock:TwiterFriendListBlock?){
        
        twiterFriendListBlock = friendListBlock
...
//            let params = ["user_id" : userID, "cursor" : cursor as Any] as [String : Any]
            ...
            client.sendTwitterRequest(request) { (response, data, connectionError) -> Void in
                if connectionError != nil {
                    ...
                    if self.twiterFriendListBlock != nil{
                        self.twiterFriendListBlock!(nil, connectionError)
                        self.twiterFriendListBlock = nil
                    }
                }
                ...
                do {
                    if data != nil{
                        ...
                        if let result = json as? [String : Any]{
                            if self.twiterFriendListBlock != nil{
                                self.twiterFriendListBlock!(result, connectionError)
                                self.twiterFriendListBlock = nil
                            }
                        }
                    }
                    ...
                    if self.twiterFriendListBlock != nil{
                        self.twiterFriendListBlock!(nil, jsonError)
                        self.twiterFriendListBlock = nil
                    }
                }
            }
        }else{
            self.twitterLogin(fromVC: viewController) { (reponse, error) in
                if reponse != nil{
                    self.twitterFriendList(cursor : cursor, fromVC: viewController, friendListBlock: friendListBlock)
                }
            }
        }
    }
    ...
    func sendDirectMessage(userID : String?, fromVC viewController : UIViewController?,friendListBlock:TwiterFriendListBlock?){
        twiterFriendListBlock = friendListBlock

        if let userLoginID = TWTRTwitter.sharedInstance().sessionStore.session()?.userID {
            let client = TWTRAPIClient(userID: userLoginID)
            ...
            let params = [
                "event": [
                    ...
                    "message_create": [
                        ...
                        "message_data": ["text": "Sevenchats Invations"]
                    ]
                ]
            ]
            ...
            self.twitterLogin(fromVC: viewController) { (reponse, error) in
                if reponse != nil{
                    self.sendDirectMessage(userID: userID, fromVC: viewController, friendListBlock: friendListBlock)
                }
            }
        }
    }
