...
class SocketIOManager: NSObject {
    ...
    func registerForRemoteNotifictionViaSocket() {
        ...
        socketIOClient?.onAny { event in
            ...
            for evntinfo in eventNotificationInfo{
                let eventConvertObj = evntinfo as? [String:Any] ?? [:]
                print("-------\(eventConvertObj)")
                let content = eventConvertObj["content"] as? String
                
                let converfirst = content?.replacingOccurrences(of: "  \\\"", with: "\"")
                let convertsecond = converfirst?.replacingOccurrences(of: " \\\"", with: "\"")
                let convertthird = convertsecond?.replacingOccurrences(of: "\\\" ", with: "\"")
                let convertfinal = convertthird?.replacingOccurrences(of: "\\\"", with: "\"")
                print("-----convert\(convertfinal)")
                let dictNot =  self.convertToDictionaryToConent(from: convertfinal ?? "")
                let  senderID = dictNot["sender_id"] as? String ?? ""
                let  type = dictNot["type"] as? String ?? ""
                print("-----------type\(type)")
                
                guard let userID = appDelegate.loginUser?.user_id else { return }
                let encrypt = EncryptDecrypt.shared().encryptDecryptModel(userResultStr: userID.description)
                if type.toInt == 2{
                    DispatchQueue.main.async(execute: {
                        appDelegate.window.rootViewController?.presentAlertViewWithOneButton(alertTitle: "", alertMessage: CDeleteContent, btnOneTitle: CBtnOk, btnOneTapped: {_ in
                            appDelegate.logOut()
        ...
        socketIOClient?.on(userId) { data, ack in
            ...
            if let notifications = data as? [[String:AnyObject]]{
                for notify in notifications{
                    ...
                    let userDelete = notify["type"] as? String
                    if userDelete?.toInt == 2{
