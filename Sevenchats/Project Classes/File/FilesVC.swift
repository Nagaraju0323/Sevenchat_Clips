...
//        }
//
//        apiTask = APIRequest.shared().userStorage(showLoader: false) { [weak self](response, error) in
...
//                    self.storage = MDLStorage(fromDictionary: data)
//                    self.progressUpgrade.setProgress(Float((self.storage?.percent ?? 0.0) / 100.0), animated: false)
//                    self.lblStoragSpace.text = "\(CUsing) \(self.storage?.uploadedUnit ?? "") \(COf) \(self.storage?.totalSpaceUnit ?? "")"
//                }
//            }
//        }
//    }
...
//                    self.getUserStorageSpace(isLoader: isLoader)
//                }
//            }else{
//                MILoader.shared.hideLoader()
//            }
...
//            if response != nil {
//                GCDMainThread.async {
//                    self.getUserStorageSpace(isLoader: false)
//                    self.arrFiles = self.arrFiles.filter({$0.id != folderId})
//                    self.tblVMyFiles.reloadData()
//                    self.deleteFolderInLocalStorage(folderID: "\(folderId)")
//                }
//            }
//        }
//    }
...
//                        self.arrFiles.append(MDLCreateFolder(fromDictionary: obj))
//                    }
//                    self.arrSortFiles = self.arrFiles.sorted(by: { $0.folderName.lowercased() < $1.folderName.lowercased() })
//                    self.tblVMyFiles.reloadData()
//                    self.getUserStorageSpace(isLoader: isLoader)
//
//                }
