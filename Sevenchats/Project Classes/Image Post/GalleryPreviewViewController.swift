...
//
//        //Call api here....
//        APIRequest.shared().addEditImagePost(para: apiPara, images: arrImageData, shoudShowLoader : true) { [weak self] (response, error) in
...
//        }
//
////        if imagePostType == .addImagePost{
////            self.navigationController?.popToRootViewController(animated: true)
////            MIToastAlert.shared.showToastAlert(position: .bottom, message: "Image uploading…")
////        }
//
//    }
//
...
//                    chatVC.arrSelectedMediaForChat = arrSelectedImages
...
//                }
//
//                /*if "\(self.navigationController?.viewControllers[0].storyboard?.value(forKey: "name") ?? "")" == "Group"{
//                    if let groupChatVC = self.navigationController?.viewControllers.first(where: {$0 is GroupChatDetailsViewController}) {
//                        let chatVC = groupChatVC as! GroupChatDetailsViewController
//                        chatVC.arrSelectedMediaForChat = arrSelectedImages
//                        self.navigationController?.popToViewController(groupChatVC, animated: true)
//                    }
//                }   // Move back on user chat detail screen
//                else if "\(self.navigationController?.viewControllers[0].storyboard?.value(forKey: "name") ?? "")" == "Chat"{
//                    if let userChatVC = self.navigationController?.viewControllers.first(where: {$0 is UserChatDetailViewController}) {
//                        let chatVC = userChatVC as! UserChatDetailViewController
//                        chatVC.arrSelectedMediaForChat = arrSelectedImages
//                        self.navigationController?.popToViewController(userChatVC, animated: true)
//                    }
//                }*/
//            }else{
//
//                if (btnInviteGroup.isSelected || btnInviteContacts.isSelected) && arrSelectedGroupFriends.count == 0 {
...
extension GalleryPreviewViewController{
    ...
    @objc fileprivate func btnUploadClicked(_ sender : UIBarButtonItem) {
        ...
        //OLD Code check box code
//        let alertController = UIAlertController(title: "Are you Sure Want to send to this image", message: CAutodelete_change, preferredStyle: UIAlertController.Style.alert);
