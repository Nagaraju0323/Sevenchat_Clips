            ...
            if response != nil && error == nil {
                ...
                resp.forEach { item in
                    ...
                    if dictcontent?["type"] == "image"{
                        ...
                        self.messagePaylaodLast(arrUser: ["\(senders )"], channelId: self.group_id , message: txtmsg, messageType: .image, chatType: .group, groupID: self.group_id, latitude: 0.0, longitude: 0.0, address: "", forwardedMsgId: "", cloleFile: nil, sender: senders , isSelected: true,createat: create ?? "",timestampDate:timestamp2 ?? "",senderName:senderName,SenderProfImg:senderProfImg)
                        UserDefaultHelper.userChatLastMsg = true
                        
                    }else if dictcontent?["type"] == "video"{
                        
                        self.messagePaylaodLast(arrUser: ["\(senders )"], channelId: self.group_id , message: txtmsg, messageType: .video, chatType: .group, groupID: self.group_id, latitude: 0.0, longitude: 0.0, address: "", forwardedMsgId: "", cloleFile: nil, sender: senders , isSelected: true,createat: create ?? "",timestampDate:timestamp2 ?? "",senderName:senderName,SenderProfImg:senderProfImg)
                        UserDefaultHelper.userChatLastMsg = true
                        
                    }else if dictcontent?["type"] == "audio"{
                        self.messagePaylaodLast(arrUser: ["\(senders )"], channelId: self.group_id , message: txtmsg, messageType: .audio, chatType: .group, groupID: self.group_id, latitude: 0.0, longitude: 0.0, address: "", forwardedMsgId: "", cloleFile: nil, sender: senders , isSelected: true,createat: create ?? "",timestampDate:timestamp2 ?? "",senderName:senderName,SenderProfImg:senderProfImg)
                        UserDefaultHelper.userChatLastMsg = true
                        
                    }else {
                        
                        self.messagePaylaodLast(arrUser: ["\(senders )"], channelId: self.group_id , message: txtmsg, messageType: .text, chatType: .group, groupID: self.group_id, latitude: 0.0, longitude: 0.0, address: "", forwardedMsgId: "", cloleFile: nil, sender: senders , isSelected: true,createat: create ?? "",timestampDate:timestamp2 ?? "",senderName:senderName,SenderProfImg:senderProfImg)
                        UserDefaultHelper.userChatLastMsg = true
                    }
...
extension GroupChatDetailsViewController : SwiftStompDelegate{
