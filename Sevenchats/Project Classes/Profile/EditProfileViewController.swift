...
//        txtDOB.text = DateFormatter.shared().string(fromDate: date!, dateFormat: "dd MMM yyyy")
//
...
//    func editProfile() {
//
//        //let dob = DateFormatter.shared().convertDateIntoGMTDate(dateToConvert: txtDOB.text, formate: "dd MMM yyyy")
//
//        let dob = DateFormatter.shared().convertDate(strDate: txtDOB.text, formate: "dd MMM yyyy")
//
//        var dict : [String : Any] = [
//            CFirstname : txtFirstName.text ?? "",
//            CLastname : txtLastName.text ?? "",
//            CDob : dob ?? "",
//            CGender : Int(Int16((appDelegate.loginUser?.gender)!))
//        ]
//
//        if (appDelegate.loginUser?.email ?? "") != (txtEmail.text ?? ""){
//            dict[CEmail] = txtEmail.text ?? ""
//        }
//
//        if (appDelegate.loginUser?.mobile ?? "") != (txtMobileNumber.text ?? ""){
//            dict[CMobile] = txtMobileNumber.text ?? ""
//            dict[CCountry_id] = self.countryCodeId
//        }
//        if NSNumber(value: appDelegate.loginUser?.country_code_id ?? 0).intValue != self.countryCodeId{
//            dict[CMobile] = txtMobileNumber.text ?? ""
//            dict[CCountry_id] = self.countryCodeId
//        }
//
//        if (countryID ?? 0) != 0{
//            dict["country"] = self.countryID!
//        }
//        if (stateID ?? 0) != 0{
//            dict["state_id"] = self.stateID!
//        }
//        if (cityID ?? 0) != 0{
...
//        print(dict)
//
//        APIRequest.shared().editProfile(dict: dict as [String : AnyObject]) { [weak self] (response, error) in
//            guard let self = self else { return }
//
...
// /*  keepchange later
//        APIRequest.shared().uploadUserProfile(userID: Int(userID), imgProfile: self.imgUser.image) { [weak self] (response, error) in
//            guard let self = self else { return }
//            if let _response = response as? [String : AnyObject], error == nil {
...
//            self.strCity = place.city ?? ""
//            /*if let arrays = place.addressComponents {
//                for i in 0..<arrays.count {
//                    let addressInfo = arrays[i]
//                    if (addressInfo.type == "administrative_area_level_2") {
//                        self.strCity = addressInfo.name
//                    }
//                }
//            }*/
//        }*/
...
//            let profile = profileImgUrl
//            self.profileImgUrl = profile
//        }else{
//            profileImgUrl = appDelegate.loginUser?.profile_img ?? ""
//        }
//
//        if isSelectedCover == true {
//            let cover = coverImgUrl
//            self.coverImgUrl = cover
//        }else{
//            coverImgUrl = appDelegate.loginUser?.cover_image ?? ""
//        }
...
//            "dob":dobconvert ?? "",
//            "short_biography":"hello new world",
//            "relationship":"Mother",
//            "profession":"good",
//            "address_line1":"home sweet",
//            "latitude":"100",
//            "longitude":"20",
//            "user_type":"1",
//            "lang_name":langName,
...
//                    self.navigationController?.popViewController(animated: true)
 ...
 //  keepchange later
       /* APIRequest.shared().uploadUserProfile(dict: dict as [String : AnyObject]) { [weak self] (response, error) in
            guard let self = self else { return }
            if let _response = response as? [String : AnyObject], error == nil {
                guard let dict = _response.valueForJSON(key: CJsonMeta) as? [String : AnyObject] else{
                    return
