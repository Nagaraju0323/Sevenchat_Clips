...
extension EventListViewController {
    ...
    func loadEventList(showLoader : Bool) {
        ...
        apiTask = APIRequest.shared().getEventList(parameters:param, showloader: showLoader) { (response, error) in
            
            self.refreshControl.endRefreshing()
            self.tblEventList.tableFooterView = nil
            
            if response != nil && error == nil {
                
                //...Remove all data when currentPage == 1
                if self.currentPage == 1{
                    self.arrEvent.removeAll()
                    self.tblEventList.reloadData()
                }
                
                if let arrData = response?.value(forKey: CJsonData) as? [[String : Any]] {
                    
                    // Add Data here...
                    if arrData.count > 0 {
                        self.arrEvent = self.arrEvent+arrData
                        self.tblEventList.reloadData()
            ...
            self.lblNoData.isHidden = self.arrEvent.count != 0
        }
    }
    
    func loadEventDatesFromServer() {
        
        APIRequest.shared().getEventDates { (response, error) in
            if response != nil {
                if let arrData = response?.value(forKey: CJsonData) as? [[String : Any]] {
                    
                    //Add Data here...
                    if arrData.count > 0 {
                        self.arrEventDate = arrData
                        self.calendar.reloadData()
                    }
                }
            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "EventListTblCell") as? EventListTblCell {
            ...
            cell.btnProfileImg.touchUpInside { [weak self] (sender) in
                guard let _ = self else { return }
                appDelegate.moveOnProfileScreen(dict.valueForString(key: CUserId), self)
            }
