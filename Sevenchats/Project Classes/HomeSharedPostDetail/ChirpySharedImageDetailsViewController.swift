...
class ChirpySharedImageDetailsViewController: ParentViewController {
   ...
   @IBOutlet weak var imgChirpy : UIImageView!
    @IBOutlet weak var imgChirpyGIF : FLAnimatedImageView!
...
//            posted_ID = chirInfo.valueForString(key: "user_id")
           ...
           // if let sharedData = chirInfo[CSharedPost] as? [String:Any]{
            ...
            let imgExtShared = URL(fileURLWithPath:chirInfo.valueForString(key: CUserSharedProfileImage)).pathExtension
            ...
            if imgExtShared == "gif"{
                ...
                imgSharedUser.isHidden  = true
                        ...
                        self.imgSharedUserGIF.sd_setImage(with: URL(string:chirInfo.valueForString(key: CUserSharedProfileImage)), completed: nil)
...
//            blurImgView.loadImageFromUrl(chirInfo.valueForString(key: Cimages), false)
          //  imgChirpy.loadImageFromUrl(chirInfo.valueForString(key: Cimages), false)
            ...
            if imgExtView == "gif"{
                ...
                imgChirpy.isHidden  = true
                        self.imgChirpyGIF.isHidden = false
                        self.imgChirpyGIF.sd_setImage(with: URL(string:chirInfo.valueForString(key: Cimages)), completed: nil)
                self.imgChirpyGIF.sd_cacheFLAnimatedImage = false
                        
                    }else {
                        self.imgChirpyGIF.isHidden = true
                        imgChirpy.isHidden  = false
                        imgChirpy.loadImageFromUrl(chirInfo.valueForString(key: Cimages), false)
                        _ = appDelegate.loginUser?.total_friends ?? 0
                    }
