        ...
        APIRequest.shared().addEditPost(para: apiPara, image: nil, apiKeyCall: CAPITagshouts) { [weak self] (response, error) in
            ...
            if response != nil && error == nil{
                ...
                if let postInfo = response![CJsonData] as? [String : Any]{
                    ...
                    if self.isFromEdit{
                        MIGeneralsAPI.shared().refreshPostRelatedScreens(postInfo,postID, self, .editPost)
                    }else{
                        MIGeneralsAPI.shared().refreshPostRelatedScreens(postInfo,postID, self, .addPost)
                    }
                }
            }
        }
